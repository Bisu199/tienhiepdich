...
chrome.runtime.onInstalled.addListener(function (details) {
    ...
    chrome.storage.local.get(
        ...
        function (items) {
            ...
            if (!items.targetLanguage) toSet.targetLanguage = 'vi';
...
function createTranslationPrompt(jsonPayload, targetLanguage) {
    const xianxiaRules = targetLanguage.toLowerCase().includes('viet') || targetLanguage.toLowerCase().includes('vi') ? `
## Quy tắc dịch tiên hiệp (QUAN TRỌNG — áp dụng khi dịch sang tiếng Việt)

1. THUẬT NGỮ TU LUYỆN & CẢNH GIỚI — giữ nguyên Hán Việt: 灵气=linh khí, 丹田=đan điền, 渡劫=độ kiếp, 元婴=nguyên anh, 金丹=kim đan, 筑基=trúc cơ, 法宝=pháp bảo, 功法=công pháp, 宗门=tông môn, 飞升=phi thăng, 天劫=thiên kiếp...
2. TÊN RIÊNG — phiên âm Hán Việt cho nhân vật, địa danh, môn phái, pháp khí, chiêu thức.
3. THÀNH NGỮ & ĐIỂN CỐ — dùng tương đương Hán Việt: 凤毛麟角=phượng mao lân giác, 卧虎藏龙=ngọa hổ tàng long...
4. XƯNG HÔ — dùng "ta"/"ngươi" thay vì "tôi/anh/em/chị". Gọi người lớn/tôn trọng bằng Hán Việt: "Nguyệt Thạch tỷ" thay vì "chị Nguyệt Thạch".
5. VĂN PHONG — câu ngắn, dứt khoát, đúng nhịp tiên hiệp. Giữ không khí huyền bí, hoành tráng.
` : '';
